    ...
    return (
        <div className="space-y-6">
            <GlassCard>
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    <div>
                        ...
                        <p className="text-sm font-bold text-slate-800/70 mt-1">
                            Log perubahan penting (harga/stok/status), plus tindakan admin.
                        </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                        <select
                            ...
                            className="px-4 py-2 rounded-2xl dr-glass border border-white/40 text-sm font-black outline-none focus:ring-2 focus:ring-emerald-500/25 focus:border-emerald-500/40"
                        >
                        ...
                        <div className="flex items-center gap-2 px-3 py-2 rounded-2xl dr-glass border border-white/40">
                            <Search size={16} className="text-slate-500" />
                            <input
                                value={query}
                                onChange={e => setQuery(e.target.value)}
                                placeholder="Search action/target/payload..."
                                className="w-56 text-sm font-bold text-slate-900 placeholder:text-slate-500/80 outline-none bg-transparent"
                            />
                        </div>
                        <button
                            type="button"
                            ...
                            className="px-4 py-2 rounded-2xl dr-glass border border-white/40 text-sm font-black text-slate-800 transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-50 inline-flex items-center justify-center gap-2"
                            title="Export CSV"
                        >
                            <Download size={16} />
                            Export
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="px-4 py-2 rounded-2xl dr-glass border border-white/40 text-sm font-black text-slate-800 transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-60 inline-flex items-center justify-center gap-2"
                        >
                            <RefreshCw size={16} />
                ...
                <div className="mt-6 overflow-auto rounded-3xl dr-glass border border-white/40">
                    <table className="min-w-[1200px] w-full text-sm">
                        <thead className="bg-white/25 text-slate-800/70">
                            <tr>
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-4 py-10 text-center text-slate-700/70 font-bold">
                                        {loading ? 'Memuat…' : 'Tidak ada log.'}
                                    </td>
                                </tr>
                            ) : (
                                filtered.map(r => (
                                    <tr key={r.id} className="border-t border-white/25 hover:bg-white/20 transition-colors">
                                        <td className="px-4 py-3 text-xs text-slate-800/70 whitespace-nowrap">
                                            {new Date(r.created_at).toLocaleString('id-ID')}
                                        </td>
                                        <td className="px-4 py-3 text-xs text-slate-900/90">
                                            {r.actor_id ? (
                                                <span title={r.actor_id} className="font-bold">
                                                    {actorNameById.get(r.actor_id) || r.actor_id}
                                                </span>
                                            ) : (
                                                <span className="text-slate-700/60">system</span>
                                            )}
                                        </td>
                                        <td className="px-4 py-3 font-black text-slate-900">{r.action}</td>
                                        <td className="px-4 py-3 text-xs text-slate-900/80">
                                            <div className="font-mono">{r.target_table || '-'}</div>
                                            {r.target_id && (
                                                <div className="font-mono text-[11px] text-slate-800/60 break-all">{r.target_id}</div>
                                            )}
                                        </td>
                                        <td className="px-4 py-3 text-xs text-slate-800/70">
                                            <pre className="max-w-[520px] whitespace-pre-wrap break-words bg-white/20 border border-white/25 rounded-2xl p-3 overflow-hidden">
