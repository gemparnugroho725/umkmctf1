...
const severityBadge = (severity: number) => {
    if (severity >= 3) return { text: 'High', cls: 'bg-rose-100/70 text-rose-800 border border-rose-200/60' };
    if (severity === 2) return { text: 'Med', cls: 'bg-amber-100/70 text-amber-800 border border-amber-200/60' };
    return { text: 'Low', cls: 'bg-white/35 text-slate-800 border border-white/40' };
};
        ...
        <div className="space-y-6">
            <GlassCard>
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    <div>
                        ...
                        <p className="text-sm font-bold text-slate-800/70 mt-1">
                            Deteksi cepat: stok negatif, pending terlalu lama, listing pending, dll.
                        </p>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="shrink-0 px-4 py-2 rounded-2xl dr-glass border border-white/40 text-sm font-black text-slate-800 transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-60 inline-flex items-center gap-2"
                    >
                        <RefreshCw size={16} />
                ...
                <div className="mt-6 space-y-4">
                    {grouped.length === 0 ? (
                        <div className="text-center py-12 text-slate-800/70 font-bold">{loading ? 'Memuat…' : 'Tidak ada anomaly.'}</div>
                    ) : (
