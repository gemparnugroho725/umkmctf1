...
const apiBaseUrl = String(
    ...
    const normalizeRouteOptions = (rawData: any, buyers: Array<{ name?: string; demand?: number; lat?: number; lng?: number }>) => {
        const buyerNames = buyers.map(b => b.name).filter(Boolean) as string[];
        const baseSequence = buyerNames.length > 0 ? ['Gudang Anda', ...buyerNames, 'Gudang Anda'] : ['Gudang Anda'];
        ...
        const normalizeSequence = (sequence: any[]) => {
            ...
            const ensureOriginFirst = !normalizeKey(normalized[0]).includes('gudang') ? ['Gudang Anda', ...normalized] : normalized;
            // Buang duplikat "Gudang" yang nyasar di tengah (biar tidak jadi Gudang -> Gudang -> buyer ...)
            const withOrigin = ensureOriginFirst.filter((step, idx, arr) => {
                const isGudang = normalizeKey(String(step)).includes('gudang');
                if (!isGudang) return true;
                // selalu keep index 0, dan keep last (kalau memang gudang)
                if (idx === 0) return true;
                if (idx === arr.length - 1) return true;
                return false;
            });

            const keys = new Set(withOrigin.map(s => normalizeKey(s)));

            // Pastikan semua buyer terpilih masuk ke rute (AI kadang lupa 1-2 nama)
            buyerNames.forEach(name => {
                const k = normalizeKey(name);
                if (!keys.has(k)) {
                    withOrigin.push(name);
                    keys.add(k);
                }
            });

            // Pastikan rute kembali ke gudang (round-trip)
            const lastKey = normalizeKey(withOrigin[withOrigin.length - 1]);
            if (!lastKey.includes('gudang')) {
                withOrigin.push('Gudang Anda');
            }

            // Pastikan tidak ada Gudang dobel berurutan (contoh: Gudang, Gudang, Buyer...)
            for (let i = withOrigin.length - 2; i >= 0; i--) {
                const a = normalizeKey(withOrigin[i]);
                const b = normalizeKey(withOrigin[i + 1]);
                if (a.includes('gudang') && b.includes('gudang')) {
                    withOrigin.splice(i + 1, 1);
                }
            }
        ...
        const toBuyerOrderKey = (sequence: string[]) =>
            sequence
                .map(s => String(s || '').trim())
                .filter(Boolean)
                .filter(s => !normalizeKey(s).includes('gudang'))
                .map(s => normalizeKey(s))
                .join('|');

        const buyerCoordMap = new Map<string, { lat: number; lng: number }>();
        buyers.forEach(b => {
            ...
            buyerCoordMap.set(normalizeKey(name), { lat, lng });
        });
        ...
        const approxRouteScoreKm = (sequence: string[]) => {
            ...
            const seller = sellerBaseCoords;
            let total = 0;
            let prev = seller;
            for (const name of order) {
                const coord = buyerCoordMap.get(normalizeKey(name));
                if (!coord) continue;
                total += haversineKm(prev, coord);
                prev = coord;
            }
            total += haversineKm(prev, seller);
            return total;
        };
        ...
        // Pastikan opsi rute berbeda berdasarkan urutan buyer (bukan cuma beda wording)
        const uniqueByBuyerOrder = new Map<string, any>();
        batches.forEach(b => {
            const seq = Array.isArray(b.sequence) ? b.sequence : [];
            const key = toBuyerOrderKey(seq);
            if (!key) return;
        ...
        const scored = candidateOrders
            .map(order => {
                ...
                return { seq, score: approxRouteScoreKm(seq) };
            })
    ...
    const handleRunAgent = async () => {
        ...
        try {
            ...
            // Preferred trusted reference: PIHPS (Bank Indonesia) for strategic commodities (requires backend proxy)
            try {
                if (String((import.meta as any).env?.VITE_API_BASE_URL || '').trim()) {
            ...
            const msg = String(error?.message || 'Workflow failed');
        ...
        <div className="flex min-h-dvh bg-slate-50 overflow-hidden font-sans">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
                ...
                <div className="p-4 sm:p-6 max-w-7xl mx-auto w-full space-y-6 pb-24 lg:pb-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {activeTab === 'dashboard' && (
                            <div className="lg:col-span-3 animate-slide-up stagger-1">
                                ...
                                {!loading && currentStep === 2 && !priceData && (
                                    <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-4">
                                        <div className="text-sm font-black text-rose-900">Analisis harga gagal.</div>
                                        <div className="mt-1 text-xs text-rose-800">
                                            {workflowError || 'Tidak ada output dari AI. Coba jalankan ulang atau cek koneksi backend/API.'}
                                        </div>
                                        <div className="mt-3 text-[11px] text-rose-700">
                                            Tips: pastikan `VITE_API_BASE_URL` mengarah ke backend (contoh `http://localhost:5001`) dan backend sedang jalan.
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        {activeTab === 'logistics' && (
                            <div className="animate-slide-up stagger-3">
                                <TerminalLogger logs={logs} onExport={handleExportLog} compact className="h-full" />
                            </div>
                        )}

                        {activeTab === 'logistics' && (
                            <div className="lg:col-span-2 animate-slide-up stagger-3 space-y-6">
                                <ShipmentCostCard
                    ...
                    {activeTab === 'dashboard' && (logs.length > 0 || !!workflowError) && (
                        <TerminalLogger logs={logs} onExport={handleExportLog} compact className="mt-6" />
                    )}
