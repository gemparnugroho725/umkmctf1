...
const LogisticsOrchestrator: React.FC<LogisticsOrchestratorProps> = ({ data, isLoading, suppliers, buyers = [], onRouteStatsChange }) => {
    ...
    const routeOptions = useMemo(() => {
        ...
        const options = directions.routes.map((route, index) => {
            const totals = getRouteTotals(route);
            const polylinePoints = (route as any)?.overview_polyline?.points || '';
            return {
                index,
        ...
        for (const opt of options) {
            const sig = `${opt.summary}|${Math.round(opt.distanceMeters / 50)}|${Math.round(opt.durationSeconds / 30)}|${String(opt.polylinePoints).slice(0, 48)}`;
    ...
    useEffect(() => {
        ...
        const exists = routeOptions.some(opt => opt.index === selectedRouteIndex);
        if (!exists) {
            setSelectedRouteIndex(routeOptions[0].index);
        }
    ...
    const roundtripSequenceStops = useMemo(() => {
        if (activeSequenceStops.length < 2) return activeSequenceStops;
        const origin = activeSequenceStops[0];
        const last = activeSequenceStops[activeSequenceStops.length - 1];
        const sameAsOrigin = Math.abs(origin.lat - last.lat) < 1e-6 && Math.abs(origin.lng - last.lng) < 1e-6;
        if (sameAsOrigin) return activeSequenceStops;
        return [...activeSequenceStops, origin];
    }, [activeSequenceStops]);

    const activeSequencePoints = useMemo(
        () => roundtripSequenceStops.map(s => ({ lat: s.lat, lng: s.lng })),
        [roundtripSequenceStops]
    );

    const displayedStopNames = useMemo(() => {
        if (roundtripSequenceStops.length < 2) return [];
        const origin = roundtripSequenceStops[0];
        const destination = roundtripSequenceStops[roundtripSequenceStops.length - 1];
        const waypoints = roundtripSequenceStops.slice(1, -1);

        if (!optimizeWaypoints || !routeStats.waypoint_order || waypoints.length === 0) {
            return roundtripSequenceStops.map(s => s.name);
        }
    ...
    }, [optimizeWaypoints, routeStats.waypoint_order, roundtripSequenceStops]);
        ...
        <GlassCard className="h-full flex flex-col">
            ...
            <div className="h-[320px] sm:h-[380px] lg:h-[420px] xl:h-[520px] rounded-xl overflow-hidden border border-slate-200 mb-6 z-0 relative">
                {!apiKey ? (
                    ...
                    >
                        ...
                        {(directions ? displayedStops : roundtripSequenceStops).map((stop, idx, arr) => {
                            const isFirst = idx === 0;
                            const isLast = idx === arr.length - 1;
                            const isSeller = isFirst || isLast;
                            const buyerIndex = Math.max(0, Math.min(idx - 1, arr.length - 3));
                            const firstWord = String(stop.name || '').trim().split(/\s+/)[0] || String(stop.name || '');
                            const labelText = isSeller ? 'Seller' : `(${buyerIndex + 1}) ${firstWord}`;
            ...
            <div className="space-y-2">
                {activeData.batches.map((batch) => {
                    ...
                    const rawSequence = Array.isArray(batch.sequence) ? batch.sequence.filter(Boolean) : [];
                    const sequence =
                        rawSequence.length > 1 && rawSequence[0] !== rawSequence[rawSequence.length - 1]
                            ? [...rawSequence, rawSequence[0]]
                            : rawSequence;
                    const firstStop = sequence[0] || 'Gudang Anda';
                    const lastStop = sequence[sequence.length - 1] || 'Gudang Anda';
                    const midStops = Math.max(0, sequence.length - 2);
                    return (
                    <div
                        key={batch.batch_id}
                        ...
                        <div className="flex items-start justify-between gap-2 px-3 py-2">
                            ...
                            >
                                ...
                                <div className="text-[10px] text-slate-400">
                                    {midStops > 0 ? `${firstStop} → ${midStops} titik → ${lastStop}` : `${firstStop} → ${lastStop}`}
                                </div>
                                <div className="text-[10px] text-slate-400">
                                    {batch.est_distance_km || '?'} km • {batch.est_time_mins || '?'} mnt • {sequence.length || 0} titik
                                </div>
                            </button>
                            <div className="flex items-center gap-1">
                                {isSelected && (
                                    ...
                                    {sequence.map((step, idx) => (
                                        <div key={idx} className="flex items-start gap-2">
                                            <div className="flex flex-col items-center">
                                                ...
                                                {idx !== sequence.length - 1 && <div className="w-0.5 h-3 bg-slate-200"></div>}
                                            </div>
                                            <div className="flex flex-col flex-1">
                                                <span className="text-[10px] font-bold text-slate-700">{step}</span>
                                                <span className="text-[8px] text-slate-400">
                                                    {idx === 0 ? 'Titik Mulai' : idx === sequence.length - 1 ? 'Kembali ke Gudang' : 'Tujuan Pengiriman'}
                                                </span>
                                            </div>
                                        </div>
                                    ))}
